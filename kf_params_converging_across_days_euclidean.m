...
idx = 7:28;
perf_idx = [4,6,6,7,7,8,9,11,12,12,13,14,...
    15,15,16,16,17,18,19,19,20,20] - 3;
...
for i=1:length(session_break)-1,
    ...
    for t=idx,
        x_dist(end+1) = norm(Cx(:,t)-Cx(:,ref_idx))/abs(mean(Cx(:,ref_idx)));
        y_dist(end+1) = norm(Cy(:,t)-Cy(:,ref_idx))/abs(mean(Cy(:,ref_idx)));
        c_dist(end+1) = norm(Cc(:,t)-Cc(:,ref_idx))/abs(mean(Cc(:,ref_idx)));
        tot_dist(end+1) = mean([x_dist(t),y_dist(t),c_dist(t)]);
    end
    x_dist(end+1) = nan;
    y_dist(end+1) = nan;
    c_dist(end+1) = nan;
...
h(3)=plot(c_dist);
h(4)=plot(tot_dist);
ylabel({'Dist btw Kalman Weights','(current to last session)'})
...
h(3)=plot(c_dist);
h(4)=plot(tot_dist);
ylabel({'Dist btw Kalman Weights','(current to last in day)'})
...
h(4)=plot(tot_dist);
xlabel('trials')
ylabel({'Dist btw Kalman Weights','(current to last overall)'})
...
legend(h,{'Vx Weights','Vy Weights','Constant Weights','Avg Dist'})
