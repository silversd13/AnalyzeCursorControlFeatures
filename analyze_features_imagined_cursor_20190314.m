...
filename = '20190314-144445-002.ns2';
...
pathname = '/media/dsilver/data/Bravo1/20190314/GangulyServer/Center-Out/20190314/150120/Imagined';
...
%% reduce trials
trials = trials(1:length(datafiles));
...
for target=1:4,
    ...
    for ch=1:Nch,
        ...
        for i=1:length(fpass_starts),
            erps(:,:,i) = createdatamatc(squeeze(pwr(:,ch,i)),E,Fs,[0,9]);
        end
        
        imagesc(ax(idx),linspace(-5,4,9000),fpass_starts,squeeze(mean(erps,2)));
        drawnow;
    end
    ...
    % save figure
    saveas(fig,sprintf('Figures/20190314-imagined/hilb-spectrogram-target-%i.png',target))
    close(fig)
end
