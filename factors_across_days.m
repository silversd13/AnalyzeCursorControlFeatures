    ...
    for ii=1:length(expt.hhmmss),
        ...
        for iii=1:T,
            ...
            if strcmp(TrialData.Events(end).Str,'Reach Target'),
                ...
                CursorState{trial} = TrialData.CursorState;
                IntendedCursorState{trial} = TrialData.IntendedCursorState;
...
for feature=1:7,
    for i=1:length(day_break)-1,
        ...
        target_day = cat(1,TargetID{day_idx});
        F_day = Fcell(day_idx);
        CS_day = CursorState(day_idx);
        ICS_day = IntendedCursorState(day_idx);
        KF_day = KF(day_idx);
        
        for t=1:8,
            % only trials for top target
            target_idx = target_day==t;
            F = cat(1,F_day{target_idx});
