...
pathname = '/media/dsilver/data/Bravo1/20190326/Blackrock/20190326-105148/';
filename = '20190326-105148-001.ns2';
...
pathname = '/media/dsilver/data/Bravo1/20190326/GangulyServer/Center-Out/20190326/105214/Imagined';
...
erps = zeros(5*Fs,length(fpass_starts),Nch,8);
for i=1:length(fpass_starts),
    ...
    for target=1:8,
        for ch=1:Nch,
            ...
            erps(:,i,ch,target) = mean(createdatamatc(pwr(:,ch),E,Fs,[0,5]),2);
        end
    end
end
...
for target=1:8,
    ...
    for ch=1:Nch,
        ...
        imagesc(ax(idx),linspace(-3,2,5000),fpass_starts,squeeze(erps(:,:,ch,target)));
        drawnow;
    end
...
%     CC = [min(CC(:)),max(CC(:))];
    ...
    % save figure
    saveas(fig,sprintf('Figures/20190326-imagined-abstract/hilb-spectrogram-target-%i.png',target))
    close(fig)
end
...
for target=1:8,
    ...
    for ch=1:Nch,
        ...
        imagesc(ax(idx),linspace(-3,2,5000),fpass_starts,...
...
%     CC = [min(CC(:)),max(CC(:))];
    ...
    % save figure
    saveas(fig,sprintf('Figures/20190326-imagined-abstract/hilb-spectrogram-deviation-target-%i.png',target))
    close(fig)
end
