function [mse_avg] = eval_channels_decoders(datadir)
...
best_decode_channels = []; %  list of channels to use in decoder (in greedy order)
curr_decode_channels = []; % current list
...
for i=1:num_channels, % all feature combinations
    ...
    best_ch = 0;
    best_mse = inf;
    for ch=setdiff(1:num_channels,best_decode_channels),
        curr_decode_channels = [best_decode_channels, ch];
    ...
    % store mse
    mse_avg(i) = best_mse;
    best_decode_channels = [best_decode_channels, best_ch];
...
saveas(fig,fullfile(savedir,'perf_across_channels'),'png')
close(fig)

%% save mat file w/ data
save(fullfile(savedir,'perf_across_channels.mat'),...
