...
pathname = '/media/dsilver/data/Bravo1/20190314/Blackrock/20190314-144445/';
filename = '20190314-144445-005.ns2';
...
while i<length(pulseidx),
    ...
    if num_pulses==3, % if 3, split into 1 and 2
        pulse_groups(ct) = 1; % time of first pulse
        group_idx(ct) = pulseidx(i-3);
        group_times(ct) = pulsetime(i-3);
        ct = ct + 1;
        
        pulse_groups(ct) = 2; % time of third pulse
        group_idx(ct) = pulseidx(i-2);
        group_times(ct) = pulsetime(i-2);
        ct = ct + 1;
...
for i=1:length(idx1),
    ...
    trials(i).mvmt_idx = group_idx(idx2(i));
    trials(i).iti_time = group_times(idx1(i));
    trials(i).mvmt_time = group_times(idx2(i));
end

%% load matlab data to get idx of trial types
pathname = '/media/dsilver/data/Bravo1/20190314/GangulyServer/Center-Out/20190314/152317/Imagined';
...
idx = (trials(1).iti_idx-round(10*Fs)):(trials(end).mvmt_idx+round(10*Fs));
time = time(idx);
anin = anin(idx);
lfp = lfp(idx,:);
...
erps = zeros(8*Fs,length(fpass_starts),Nch,4);
for i=1:length(fpass_starts),
    ...
    % store & zscore
    pwr = zscore(log10(abs(hlfp)));
    
    % smooth pwr (window of 30ms)
    for ch=1:Nch,
        pwr(:,ch) = smooth(pwr(:,ch),round(30e-3*Fs));
    end
    ...
    for target=1:4,
        for ch=1:Nch,
            ...
            erps(:,i,ch,target) = mean(createdatamatc(pwr(:,ch),E,Fs,[0,8]),2);
        end
    end
end
...
for target=1:4,
    ...
    for ch=1:Nch,
        ...
        imagesc(ax(idx),linspace(-5,3,5000),fpass_starts,squeeze(erps(:,:,ch,target)));
        drawnow;
    end
...
%     CC = [min(CC(:)),max(CC(:))];
    ...
    % save figure
    saveas(fig,sprintf('Figures/20190314-overt/hilb-spectrogram-target-%i.png',target))
    close(fig)
end

%% deviation from avg erp
for target=1:4,
    fig = figure('units','normalized','position',[.1,.1,.8,.8],...
        'name',sprintf('hilb-spectrogram-deviation-target-%i',target));
    ax = tight_subplot(R,C,[.01,.01],[.06,.01],[.03,.01]);
    set(ax,'NextPlot','add');
    ...
    for ch=1:Nch,
        ...
        imagesc(ax(idx),linspace(-5,3,5000),fpass_starts,...
            squeeze(erps(:,:,ch,target))-(squeeze(mean(erps(:,:,ch,:),4))));
        drawnow;
    end
    % make all axes the same
    XX = [-3,2];
...
%     CC = [min(CC(:)),max(CC(:))];
    ...
    % save figure
    saveas(fig,sprintf('Figures/20190314-overt/hilb-spectrogram-deviation-target-%i.png',target))
    close(fig)
end
